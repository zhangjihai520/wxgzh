$(function(){
	getCarrierAuthInfo();
//	setTimeout(function(){getPlatformTranPrice()},1000)
});
...
//	form.append("transPrice", price_true);
	...
	if(articleImg != undefined) {
		photoCompress(articleImg, {
			...
			var base64 = dataURLtoFile(base64, "chemiclaTransLicenceImage.jpg");
			form.append("chemiclaTransLicenceImage", base64);
		})
	};
	setTimeout(function() {
		setData(form)
...
function setData(form) {
	$.ajax({
		url: re_url(methods.perfectCarrierInfo),
		type: "post",
		data: form,
		cache: false,
		contentType: false,
...
function getCarrierAuthInfo() { //获取企业认证信息
	
	var url = re_url(methods.getCarrierAuthInfo);
	var data = {};
...
function data_info(data) {
	...
	$("#trans_price").val(data.transPrice+"元/吨公里");
	$("#imghead").attr("src", data.businessLicenceUrl);
	$("#articleHead").attr("src", data.chemiclaTransLicenceUrl);
	save_localStorage("bdDistrictId", data.bdDistrictId);
}

$(".clean_preview").on("click", function() {
...
function getPlatformTranPrice(){
	var url=re_url(methods.getPlatformTranPrice);
	var data={};
	var success=function(response){
		if(response.code==RETCODE_SUCCESS){
			if(response.data==""){
			...
			$("#trans_price").val(response.data+"元/吨公里")
		}
	}
	util.get_ajax(url,data,success)
}
