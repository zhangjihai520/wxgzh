...
function getInvocetpltList() { //获取发票列表
	var url = re_url(methods.getCpInvoiceTpltList);
	var data = {};
...
function invoice_list(data) {
	var invoice_list = "";
	invoice_list += '<p class="col-12 m-0 p-0 font_3 invoice_id " id="' + data.cpInvoiceTpltId + '">' + data.invoiceTitle + '</p>';
	invoice_list += '<div class="row col-12 m-0 p-0 border-bottom py-2">';
	invoice_list += '<p class="col-3 pl-0 offset-3 font_1 text-right text-danger use_box" id="' + data.cpInvoiceTpltId + '">';
	invoice_list += '<span class="fa fa-edit pr-1">' + '' + '</span>';
	invoice_list += '<span>' + '使用' + '</span>';
	invoice_list += '</p>';
	invoice_list += '<p class="col-3 pl-0  font_1 text-right orange complie_box" id="' + data.cpInvoiceTpltId + '">';
	invoice_list += '<span class="fa fa-edit pr-1">' + '' + '</span>';
	invoice_list += '<span>' + '编辑' + '</span>';
	invoice_list += '</p>';
	invoice_list += '<p class="col-3 font_1 pr-0 del_box" data-toggle="modal" data-target="#exampleModalCenter" id="' + data.cpInvoiceTpltId + '">';
...
function deleteInvocetplt(id) { //删除发票
	var url = re_url(methods.deleteCpInvoiceTplt);
	var data = {
		"cpInvoiceTpltId": id
	};
	var success = function(response) {
...
page_change(".add_btn", "addinvoice.html");
